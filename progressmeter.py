...
class ProgressMeter(object):
    ...
    def reset(self, *args, **kw):
        ...
        self.__init__(
            timestamp=timestamp, unit=unit, total=total, count=count, rate_refres=rate_refresh,
            ticks=meter_ticks)
    ...
    def _get_meter(self, *args, **kw):
        ...
        if self.count >= self.total:
            ...
            if dur < 60:
                ...
                if hours == 1:
                    est = ' completed in %.f hour %.f min %.f sec' % (
                        hours, minutes, seconds)
                else:
                    est = ' completed in %.f hours %.f min %.f sec' % (
                        hours, minutes, seconds)
            ...
            if len(self.estimated_duration) >= 1:
                ...
                if self.estimated_duration[-1] < 15:
                    exp = 1.75
                    dat = 10
                elif self.estimated_duration[-1] >= 15 and self.estimated_duration[-1] < 30:
                    exp = 1.5
                    dat = 15
                elif self.estimated_duration[-1] >= 30 and self.estimated_duration[-1] < 90:
                    exp = 1.25
                    dat = 50
                else:
                    exp = 1.00
                ...
                for i in range(0, min(len(self.estimated_duration), dat)):
                    wght_num += self.estimated_duration[
                        -dat:][i] * ((i + 1) ** exp)
                    wght_den += (i + 1) ** exp
                ...
                if est_dur < 60:
                    ...
                    if hours == 1:
                        est = ' %.f hour %02.f min %02.f sec remaining' % (
                            hours, minutes, seconds)
                    else:
                        est = ' %.f hours %02.f min %02.f sec remaining' % (
                            hours, minutes, seconds)
    ...
    def _refresh(self, *args, **kw):
        if self.switch_off:
            ...
            sys.stdout.write(
                ' ' * self.prev_meter_len + '\x08' * self.prev_meter_len)
            # Get meter text
